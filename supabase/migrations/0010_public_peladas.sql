...
alter table public.peladas
drop constraint if exists peladas_public_slug_format_check,
drop constraint if exists peladas_public_slug_required_when_public_check;

alter table public.peladas
add constraint peladas_public_slug_format_check check (
  public_slug is null or public_slug ~ '^[a-z0-9-]{3,40}$'
),
add constraint peladas_public_slug_required_when_public_check check (
  is_public = false or public_slug is not null
);

create unique index if not exists peladas_public_slug_lower_idx
on public.peladas (lower(public_slug))
where public_slug is not null;

insert into storage.buckets (id, name, public)
values ('pelada-assets', 'pelada-assets', true)
on conflict (id) do update set public = true;

drop policy if exists "authenticated users upload pelada assets" on storage.objects;
drop policy if exists "public can view pelada assets" on storage.objects;
drop policy if exists "users update own pelada assets" on storage.objects;
drop policy if exists "users delete own pelada assets" on storage.objects;

create policy "authenticated users upload pelada assets"
on storage.objects for insert
to authenticated
with check (
  bucket_id = 'pelada-assets'
);

create policy "public can view pelada assets"
...
create policy "users update own pelada assets"
...
create policy "users delete own pelada assets"
