...
export function MyPresenceCard() {
  ...
  return (
    <Card>
      <h2 className="font-semibold">Minha presenca</h2>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2 text-sm">
        ...
        {context.message ? <span className="text-zinc-600">{context.message}</span> : null}
      </div>
    </Card>
  );
}
...
export function PresenceListCard({ canManage }: { canManage: boolean }) {
  ...
  return (
    <Card>
      <h2 className="font-semibold">Presencas</h2>
      <div className="mt-4 space-y-3">
        {context.members.map((member) => (
          <PresenceRow key={member.user_id} member={member} canManage={canManage} />
        ))}
        {!context.members.length ? <p className="text-sm text-zinc-600">Nenhum membro cadastrado na pelada.</p> : null}
      </div>
    </Card>
  );
}
...
function PresenceRow({ member, canManage }: { member: Member; canManage: boolean }) {
  ...
  return (
    <div className="relative rounded-md border border-zinc-200 p-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {canManage ? (
            <button
              ...
              className="rounded-md border border-zinc-300 bg-white p-2 hover:bg-zinc-50"
      ...
      {open ? (
        <div className="absolute right-3 top-12 z-10 grid w-44 gap-1 rounded-md border border-zinc-200 bg-white p-2 shadow-soft">
          {(Object.keys(labels) as PresenceStatus[]).map((status) => {
            const Icon = icons[status];
            return (
              <button
...
function StatusBadge({ status }: { status: PresenceStatus }) {
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-semibold uppercase",
